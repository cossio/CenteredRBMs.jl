...
function centered_energy(
    rbm::RBMs.RBM, v::AbstractArray, h::AbstractArray, λv::AbstractArray, λh::AbstractArray
)
    Ev = RBMs.energy(rbm.visible, v)
    Eh = RBMs.energy(rbm.hidden, h)
    Ev .+ Eh .+ RBMs.interaction_energy(rbm, v .- λv, h .- λh) .- RBMs.interaction_energy(rbm, λv, λh)
end

function binary_center(
    rbm::RBMs.RBM{<:RBMs.Binary, <:RBMs.Binary}, λv::AbstractArray, λh::AbstractArray
)
...
@testset "grad2mean $Layer" for Layer in InteractiveUtils.subtypes(RBMs.AbstractLayer)
    ...
    @test CenteredRBMs.grad2mean(rbm.visible, ∂.visible) ≈ dropdims(Statistics.mean(v; dims=2); dims=2)
end
