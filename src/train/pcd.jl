function RBMs.pcd!(rbm::CenteredRBM, data::AbstractArray;
    ...
    wts = nothing,
    steps::Int = 1,
    vm = RBMs.transfer_sample(visible(rbm), falses(size(visible(rbm))..., batchsize)),
    stats = RBMs.suffstats(visible(rbm), data; wts),
...
function ∂contrastive_divergence_and_center!(
    ...
    offset_h_new = grad2mean(hidden(rbm), ∂d.hidden)  # <h>_d from minibatch
